class UserStocksController < ApplicationController
  ...
  def create
    if params[:stock_id].present?
      @user_stock = UserStock.new(stock_id: prams[:stock_id], user: current_user)
    else
      ...
      if stock
        ...
        if stock.save
          ...
          @user_stock = nil
          flash[:error] = "Stock '#{params[:stock_ticker].upcase}' is not available."
        end
      end
    end
    ...
    respond_to do |format|
      if @user_stock.save
        format.html { redirect_to my_portfolio_path, 
                      notice: "Stock '#{@user_stock.stock.ticker.upcase}' was successfully added." 
                    }
